...
def semantic_search_conversation(query, vectorstore):
    try:
        ...
        retriever_ = MultiQueryRetriever.from_llm(
        ...
        setup_and_retrieval = RunnableParallel(
            {"context": retriever_, "question": RunnablePassthrough()}
        )
...
def conversation_retrieval_chain(query, vectordb):
  ...
  retriever_ = MultiQueryRetriever.from_llm(
        retriever=vectordb.as_retriever(search_type="similarity", search_kwargs={"k": num_chunks}), llm = model
    )
